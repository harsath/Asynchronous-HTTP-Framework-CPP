...
TEST(HTTPMessage_parse_constructor, HTTPMessage){
	...
	std::unique_ptr<HTTPMessage> http_message = std::make_unique<HTTPMessage>();
	...
	ASSERT_EQ(http_message->ConstGetHTTPHeader()->GetHeaderCount(), 5);
	...
	ASSERT_EQ(http_parser.first, ParserState::PARSING_DONE);
	...
	http_message->SetResponseCode(HTTPConst::HTTP_RESPONSE_CODE::FORBIDDEN);
	EXPECT_TRUE(
		http_message->GetResponseCode() == HTTPConst::HTTP_RESPONSE_CODE::FORBIDDEN
		);
}
...
TEST(HTTPMessage_build_response_message, HTTPMessage){
	...
	http_message->SetResponseCode(HTTPConst::HTTP_RESPONSE_CODE::OK);
